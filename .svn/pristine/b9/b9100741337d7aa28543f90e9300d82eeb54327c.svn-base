...
import static nsf.ecap.config.IPULSE_CONSTANTS.ISO9001_CorporateTestCaseName;
import static nsf.ecap.config.IPULSE_CONSTANTS.ISO9001_SingleTestCaseName;
import static nsf.ecap.config.IPULSE_CONSTANTS.Non_Cert_StandardsTestCaseName;
import static nsf.ecap.config.IPULSE_CONSTANTS.SnapShot_ModBrack_Review_Task;
import static nsf.ecap.config.IPULSE_CONSTANTS.SnapShot_taskDesc_SampleSelection;
...
import static nsf.ecap.config.IPULSE_CONSTANTS.WOISOTaskTab_ExtDBUpdateAIType;
...
public class TestSuiteBase_FPC_Work_Order_suite extends TestSuiteBase {
	...
	public static  BS_01_FPC_Custom_Certified FPC_BS01;
	...
	//	currentSuiteName = "ISR_suite";
		setCurrentSuiteName("FPC_Work_Order_suite");
		fnpMymsg("             ");
		fnpMymsg("             ");
		fnpMymsg("=========================================================================================================================================");
		...
		if (TestUtil.isSuiteRunnable(suiteXls, currentSuiteName)) {
			...
			setCurrentSuiteExcel(FPC_Work_Order_suitexls);
		}
...
/*		
	...
	public static void fnpDB_UpdateAuditStatusToCancel(String facilityNo, String standard) throws Throwable {
		fnpMymsg("******************");
		try {
		...
		try {
			...
			String updateQuery1 = String.format("update OA_AUDIT_PERFORM ap set AP.AUDIT_STATUS = 'CANCELLED'  where AP.AUDIT_NO in ( select distinct AP.AUDIT_NO from OA_AUDIT_PERFORM ap, oa_audits a  where ap.audit_no = a.audit_no AND AP.AUDIT_STATUS IN ( 'CREATED', 'SUBMITTED','REVIEWED', 'COMPLETED') AND a.FACILITY_CODE='C0029698')");
			
			int a = stmt.executeUpdate(updateQuery1);
			fnpMymsg("PASSED == Successfully executed the Query **** "+updateQuery1+" **** and the output is= " + a);
			connection.commit();
			connection.close();
...
/*				
	...
	public static void fnpOasisPartInISO(String taskName,String visitNo) throws Throwable {
		try {
			...
			fnpClick_OR("Oasis_SaveNNextBtn");
			Thread.sleep(2000);
			fnpCheckErrorUsingPageSource_Oasis();
			...
			fnpClick_OR("Oasis_Audit_EditBttn");
			Thread.sleep(2000);
			fnpCheckErrorUsingPageSource_Oasis();
			
			fnpFillQasisQuesSet();
			
			fnpClick_OR("Oasis_Audit_QuesSetTable_SaveClose_Bttn");
			Thread.sleep(2000);
			fnpCheckErrorUsingPageSource_Oasis();
			...
			fnpSelectValueFromDD_BySelectClass("Visit Start Time", 1, -2);
			fnpSelectValueFromDD_BySelectClass("Visit Start Time", 2,  -2);
			fnpSelectValueFromDD_BySelectClass("Visit End Time", 1, -1);
			fnpSelectValueFromDD_BySelectClass("Visit End Time", 2, -1);
				
			
			fnpClick_OR("Oasis_btnSavePreviousBtn");
			Thread.sleep(2000);
			fnpCheckErrorUsingPageSource_Oasis();
			
			fnpOasisUploadAuditReport();
	...
	public static void fnpFillQasisQuesSet() throws Throwable {
		try {
			List<WebElement> RadioBttns=fnpGetORObject("Oasis_Audit_QuesSetTable").findElements(By.tagName("input"));
			for(WebElement RadioBttnselement:RadioBttns){
				try {
					if(RadioBttnselement.isDisplayed()) {
						RadioBttnselement.click();
						Thread.sleep(500);
					}
				}catch(Throwable t) {
					//nothing to do
			...
			int i=0;
			List<WebElement> Textareas=fnpGetORObject("Oasis_Audit_QuesSetTable").findElements(By.tagName("textarea"));
			for(WebElement Textareaselement:Textareas){
				i++;
				try {
					if(Textareaselement.isDisplayed()) {
						Textareaselement.click();
						Textareaselement.sendKeys("Automation "+i);
						Thread.sleep(1000);
					}
				}catch(Throwable t) {
					//nothing to do
	...
	public static void fnpOasisUploadAuditReport() throws Throwable {
		try {
			String originalHandle = driver.getWindowHandle();
			ArrayList<String> Oldtabs = new ArrayList<String>(driver.getWindowHandles());
			fnpClick_OR("Oasis_Audit_Upload0Bttn");
			Thread.sleep(4000);
			fnpCheckErrorUsingPageSource_Oasis();
			System.out.println("TEST");
			ArrayList<String> Newtabs = new ArrayList<String>(driver.getWindowHandles());
			for(int i=0; i<Newtabs.size(); i++) {
				if(Oldtabs.get(0).equalsIgnoreCase(Newtabs.get(i))) {
					//nothing to do break;
				}else if(Oldtabs.get(1).equalsIgnoreCase(Newtabs.get(i))) {
					//nothing to do break;
				}else {
			...
			fnsUploadFile("Oasis_Audit_UploadInput", "Oasis Upload");
			fnpClick_OR("Oasis_Audit_UploadPopup_UploadBttn");
			Thread.sleep(2000);
			
			fnpWaitForVisible("Oasis_Audit_UploadPopup_FileUploaded");
			
			fnpClick_OR("Oasis_Audit_UploadPopup_CloseBttn");
			Thread.sleep(2000);
...
/*		
		...
		*/
		...
		fnpSelectingAuditorThroughSAM(todaydateInStringformat2,afterOneWeekdateInStringformat);
...
/*		
	...
	public static void fnpSelectingAuditorThroughSAM(String startDateInString,String endDateInString) throws Throwable{
		...
		String xpathForAuditorLinkInSearchResultInSAM="//a[contains(text(), '"+(String) hashXlData.get("Auditor_Code").trim()+"')]";
		 ...
		 * fnsClick_OR("AssignVisitInSAM_OkContinue_Button");
		 * fnpMymsg("Clicked AssignVisitInSAM_OkContinue_Button in SAM");
		 * fnsLoading_wait_In_SAM(1); fnsLoading_wait_In_SAM(1);
		 */
...
/*		String xpathForDeleteFacilityEmployeeCountIcon=".//*[@id='mainForm:tabView:empCountDtFcInfo:0:delShiftEmpFac']";
	...
	public static void  setISOSingleObject(){
		FPC_BS01 = new BS_01_FPC_Custom_Certified();
	}
...
/*						if (waitCount > 4) {
	...
	public static String  fnsFindNClickOn_TaskOrAIlink_FromTable(String TableDataXpath, String TaskOrAIDesc, Integer columnnumber) throws Throwable {
		String AIOrTaskNumber = "";		
		boolean Taskfound = false;
		try {	
			fnpWaitForVisible(TableDataXpath);
			driver.manage().timeouts().implicitlyWait(1, TimeUnit.SECONDS);
			String TableRowsXpath = OR.getProperty(TableDataXpath)+"/tr";
			Integer TotalRows = driver.findElements(By.xpath(TableRowsXpath)).size();
			for(int i=1; i<=TotalRows; i++){
				String PerRowXpath = TableRowsXpath+"["+i+"]";
				String PerRowText = driver.findElement(By.xpath(PerRowXpath)).getText().trim().toLowerCase();
				if(PerRowText.contains(TaskOrAIDesc.toLowerCase().trim())){
					Taskfound = true;
					String AIOrTaskLinkXpath = PerRowXpath+"/td["+columnnumber+"]";
					AIOrTaskNumber = driver.findElement(By.xpath(AIOrTaskLinkXpath)).getText().trim();
					driver.findElement(By.xpath(AIOrTaskLinkXpath)).click();
					driver.manage().timeouts().implicitlyWait(Long.parseLong(CONFIG.getProperty("implicitWaitTime")), TimeUnit.SECONDS);
					fnpMymsg("PASSED == Successfully clicked on '"+AIOrTaskNumber+"' link.");
					fnpCheckError(AIOrTaskNumber);
					fnpLoading_wait();					
					break;
				}
			}	
			if(Taskfound==false){
				throw new Exception ("FAILED == '"+TaskOrAIDesc+"' is not coming");
			}
			return AIOrTaskNumber;
		}catch (Throwable t) {
			throw new Exception (Throwables.getStackTraceAsString(t));
		}
	}
	...
	public static void fnpSampleSelectionEPSFCode() throws Throwable {
		...
		if (classNameText.equalsIgnoreCase("Modbrack_Not_Certified")) {
			int modBrackrowNo = fnpFindRow("Snapshot_TasksSummaryTable", SnapShot_ModBrack_Review_Task, TaskDescColIndex);
	...
	public static void fnpSelectValueFromDD_BySelectClass(String DD_LabelName, int SeqOfSelect, int IndexValue_MinusFromBottomPluseFromTop) throws Exception {
		try {
			String DD_Xpath = "//*[contains(text(), '"+DD_LabelName+"')]/following::select["+SeqOfSelect+"]";
			Select DD_Element = new Select (fnpGetORObjectX___NOR(DD_Xpath));
			int DD_Size = DD_Element.getOptions().size(); 
			if(IndexValue_MinusFromBottomPluseFromTop>=0) {
				DD_Element.selectByIndex(IndexValue_MinusFromBottomPluseFromTop);
			}else {
				DD_Element.selectByIndex((DD_Size+IndexValue_MinusFromBottomPluseFromTop));
			}
			fnpMymsg("PASSED == successfully select index value from dd = "+DD_LabelName);
		}catch(Throwable t) {
			throw new Exception(Throwables.getStackTraceAsString(t));
		}
	}
		...
		while (true) {
			try {
				...
				for (int i = 1; i < TotalRowCount + 1; i++) {
					...
					msg="Debug:: Actual value is --'"+Value+"' and expected value is --'"+matchingName+"' and are they equal --'"+Value.equalsIgnoreCase(matchingName)+"' .";
					//fnpMymsg(msg);
					APP_LOGS.debug(msg);
					if (Value.toLowerCase().trim().contains(matchingName.toLowerCase().trim())) {
						resultRowNo = i;
						break;
				...
				if (resultRowNo > 0) {
					...
					if (retries > 4) {
						throw new Exception("Row and Col no. is not calculated for '" + matchingName + "' . Row no. is '" + resultRowNo + "' and Column no. is '" + colNo + "' . Either it is not present in table or may be some error in code.");
					} else {
	...
	public void fnpDropWOFPCandDeleteSomeDataFromDB(String facilityNo) throws Throwable {
		...
		try {
			...
			String extractWOQuery = String.format("SELECT iacx.app_no FROM IQ.IQ_APP_CSX_XREF iacx, NUCLEUS.CUS_STD_XREF csx,IQ.IQ_APPLICATIONS apps, customers cus   where cus.seq= apps.cus_seq and iacx.cussx_seq = csx.seq and apps.app_no= iacx.app_no and APPS.APP_STATUS not in ('DROP','COMPLETED') and cus.code ='%s'",facilityNo);
						
			String woNo="";        
	        ResultSet rs = stmt.executeQuery(extractWOQuery);
	        while (rs.next()) {
	             woNo = rs.getString("APP_NO");
	        	        
		        if (!(woNo.trim().equalsIgnoreCase(""))) {
		        	String updateQuery1 = String.format("update oa_corr_actions cor set COR.CA_STATUS='DROPPED',MODIFY_DATE= SYSDATE, MODIFY_USER ='DROP_WO'  where COR.IQ_JOB_NO in ( select job_no from iq_app_jobs where app_no='"+woNo+"')");
					String updateQuery2 = String.format("update OA_AUDIT_PERFORM ap set AP.AUDIT_STATUS = 'CANCELLED', MODIFY_DATE= SYSDATE, MODIFY_USER ='DROP_WO'  where AP.AUDIT_NO in ( select oa.audit_no from oa_audits oa, iq_app_jobs aj where oa.iq_job_no=aj.job_no and aj.app_no= '"+woNo+"')");
					String updateQuery3 = String.format("update ECAP.AI_ACTION_ITEMS set ACTION_ITEM_STATUS='AI_CANCELLED' , MODIFY_DATE= SYSDATE, MODIFY_USER ='DROP_WO' where  ENTITY_ID = 'W0696787' or ENTITY_ID in ( select job_no from iq_app_jobs where app_no='"+woNo+"')");
					String updateQuery4 = String.format("update iq_app_jobs aj set aj.job_status='DROPPED' , MODIFY_DATE= SYSDATE, MODIFY_USER ='DROP_WO'  where aj.app_no='"+woNo+"'");
					String updateQuery5 = String.format("UPDATE IQ_APPLICATIONS  SET APP_STATUS = 'DROP', MODIFY_DATE= SYSDATE, MODIFY_USER ='DROP_WO'  WHERE APP_NO ='"+woNo+"'");
				
					
					String deleteQuery6 = String.format("Delete from CUS_FACILITY_SIZE where CUS_SEQ=(select seq from customers where code= '"+facilityNo+"')");
					String deleteQuery7 = String.format("Delete from CUS_EMPLOYEE_COUNT where CUS_SEQ=(select seq from customers where code= '"+facilityNo+"')");
					String deleteQuery8 = String.format("Delete FROM  NUCLEUS.CUS_STD_CONTACT_XREF where CSX_SEQ IN (select cussx_seq from iq_app_csx_xref aps where aps.app_no='"+woNo+"')");
					
					
					
					int a = stmt.executeUpdate(updateQuery1);
					fnpMymsg("PASSED  Query1 Executed successfully " + a +" And Query is = "+updateQuery1);
					connection.commit();

					int b = stmt.executeUpdate(updateQuery2);
					fnpMymsg("PASSED  Query2 Executed successfully " + b+" And Query is = "+updateQuery2);
					connection.commit();

					int c = stmt.executeUpdate(updateQuery3);
					fnpMymsg("PASSED  Query3 Executed successfully " + c+" And Query is = "+updateQuery3);
					connection.commit();

					int d = stmt.executeUpdate(updateQuery4);
					fnpMymsg("PASSED  Query4 Executed successfully " + d+" And Query is = "+updateQuery4);
					connection.commit();					
					
					int e = stmt.executeUpdate(updateQuery5);
					fnpMymsg("PASSED  Query5 Executed successfully " + e+" And Query is = "+updateQuery5);
					connection.commit();
					
					
					
					int f = stmt.executeUpdate(deleteQuery6);
					fnpMymsg("PASSED  Query6 Executed successfully " + f+" And Query is = "+deleteQuery6);
					connection.commit();

					int g = stmt.executeUpdate(deleteQuery7);
					fnpMymsg("PASSED  Query7 Executed successfully " + g+" And Query is = "+deleteQuery7);
					connection.commit();					
					
					int h = stmt.executeUpdate(deleteQuery8);
					fnpMymsg("PASSED  Query8 Executed successfully " + h+" And Query is = "+deleteQuery8);
					connection.commit();
					
	        	
				} else {
					fnpMymsg(" Work Order for this facility --'"+facilityNo+"' is already dropped.");
				}
	        }
		} catch (SQLException e) {
			fnpMymsg("*********************************************** Either connection Failed or some different error with Database,   Getting Error >>  " + e.getMessage().trim());
