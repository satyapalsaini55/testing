...
public class EPSF_Complete_Flow extends TestSuiteBase_EPSF {
	 //BS-07
	...
	@Test(dataProviderClass = TestDataProvider.class,dataProvider = "EPSF_Complete_Flow")
	public void Draft_tossed_returned_flow(
		...
		//	fnpClickTopHomeMenu();
			...
			fnpMymsg("Top Message after clicking Auto Login To Beaker button----" + fnpGetText_OR("TopMessage3"));
			 ...
			 fnpMymsg("Message is -----"+SuccessfulMsg);
			...
			Assert.assertTrue(SuccessfulMsg.toLowerCase().contains("logged into beaker"), "Top message does not contain 'logged into beaker' word, after clicking Auto Login To Beaker");
			fnpMymsg("'logged into beaker' word is present in successful message");
			
			
			
			String current_epsfStatus = fnpGetText_OR("EPSF_Status");
			String current_epsfStatusListValue = fnpGetText_OR("EPSFStatusPFList");
			Assert.assertTrue(current_epsfStatus.equalsIgnoreCase((String)table.get("AutoLoggedIn_ChangeStatus")), "EPSF status not changed to '"+
					table.get("AutoLoggedIn_ChangeStatus")+"'.");	
			...
			fnpMymsg("EPSF status  changed to '"+(String)table.get("AutoLoggedIn_ChangeStatus")+"'.");
	
			
			fnpClick_OR("EPSFTaskTabLink");
			
			fnpWaitForVisible("EPSFAddTaskByTemplateBtnInTaskTab");
			
			
			String varDisabledForTemplateBtn = fnpGetAttribute_OR("EPSFAddTaskByTemplateBtnInTaskTab", "disabled");
			...
			String varDisabledForTestCodeBtn = fnpGetAttribute_OR("EPSFAddTaskByTestCodeBtnInTaskTab", "disabled");
			...
			String varDisabledForAddSampleBtn = fnpGetAttribute_OR("EPSFAddSampleBtnInTaskTab", "disabled");
			fnpMymsg("Value of attribute disabled for this 'Add Sample' button is ---"+varDisabledForAddSampleBtn);
			Assert.assertEquals(varDisabledForAddSampleBtn, "true", " 'Add Sample' button should be disable at this point of time but it is enabled now.");
			fnpMymsg("'Add Sample' button is disabled now"); 
			
			
		} catch (Throwable t) {

			
			fnpCommonFinalCatchBlock( t,"  EPSF_Complete_flow  method is failed . ","EPSF_Complete_Flow") ;
